...
import { getCustomer, updateCustomer, createNote, createReminder } from "@/lib/crm.functions";
...
function CustomerDetail() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["customer", id], queryFn: () => get({ data: { id } }) });
  ...
  const updMut = useMutation({
    ...
    onSuccess: () => { toast.success("Updated"); qc.invalidateQueries({ queryKey: ["customer", id] }); },
  });
  const noteMut = useMutation({
    mutationFn: () => note({ data: { body: noteBody, customer_id: id } }),
    onSuccess: () => { setNoteBody(""); qc.invalidateQueries({ queryKey: ["customer", id] }); },
  });
  const remMut = useMutation({
    mutationFn: () => rem({ data: { title: remTitle, due_at: new Date(remDue).toISOString(), customer_id: id } }),
    onSuccess: () => { setRemTitle(""); setRemDue(""); qc.invalidateQueries({ queryKey: ["customer", id] }); },
  });

  if (isLoading || !data?.customer) return <div className="p-8"><Loader2 className="h-6 w-6 animate-spin" /></div>;
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="p-6 lg:col-span-2 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div><Label>Name</Label><Input defaultValue={customer.name ?? ""} onBlur={(e) => updMut.mutate({ name: e.target.value })} /></div>
            <div><Label>Company</Label><Input defaultValue={customer.company ?? ""} onBlur={(e) => updMut.mutate({ company: e.target.value })} /></div>
            <div><Label>Phone</Label><Input defaultValue={customer.phone ?? ""} onBlur={(e) => updMut.mutate({ phone: e.target.value })} /></div>
            <div>
              <Label>Status</Label>
              <Select value={customer.status} onValueChange={(v) => updMut.mutate({ status: v })}>
            ...
            <div className="sm:col-span-2"><Label>Address</Label><Textarea defaultValue={customer.address ?? ""} onBlur={(e) => updMut.mutate({ address: e.target.value })} /></div>
          </div>
        </Card>
        ...
        <Card className="p-6 space-y-3">
          ...
          <Button onClick={() => noteMut.mutate()} disabled={!noteBody.trim()} size="sm">Save</Button>
          ...
          <Button onClick={() => remMut.mutate()} disabled={!remTitle || !remDue} size="sm">Create</Button>
        </Card>
        ...
        <Card className="p-6">
          ...
          {notes.map((n) => (
            <div key={n.id} className="mb-2 rounded border border-border p-3 text-sm">
          ...
          {reminders.map((r) => (
            <div key={r.id} className="mb-2 rounded border border-border p-3 text-sm">
              <div className="font-medium">{r.title}</div>
              <div className="text-xs text-muted-foreground">{format(new Date(r.due_at), "PPp")}</div>
            </div>
          ))}
          {reminders.length === 0 && <p className="text-sm text-muted-foreground">None.</p>}
        </Card>

        <Card className="p-6 lg:col-span-3">
          <h2 className="mb-3 font-semibold">Activity</h2>
          {activity.map((a) => (
            <div key={a.id} className="flex justify-between border-l-2 border-primary pl-3 text-sm py-1">
